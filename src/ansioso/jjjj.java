...
public class jjjj {
    ...
    public static void main(String[] args) throws IOException {
        try {
            BufferedImage img = ImageIO.read(new File("C:\\Users\\baeza\\OneDrive\\Pictures\\vulkan.jpg"));
    ...
    private static void procesar(int ancho, int alto, BufferedImage img, double[][] m, int[][] mr, int[][] mg, int[][] mb) {
        ...
        marco2.getContentPane().add(new JLabel("<html>Usar 'g' para escala de grises<br>"
                                                   ...
                                                   + "Usar 'r' para reestablecer"
                                                   + "Usar 'i' para negativo<br></html>"));
        ...
        marco.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent k) {
                    ...
                    if(k.getKeyChar() == 'i'){
                    	BufferedImage imgNG = img;
                    	////////////
                    	for (int j = 0; j < ancho; j = j + 2) {
                            for (int i = alto / 2; i < alto; i++) {
                                
                            	double r = mr[i][j];
                                double g = mg[i][j];
                                double b = mb[i][j]-150;
                                double infra = (r*65536)+(g*256)+(b);
                                imgNG.setRGB(i, j, (int)infra);
                            }
                        }
                    	///////////
                        printnegativo(ancho, alto, imgNG, "Infrarojo");
                        System.out.println("Imagen -> Infrarojo");
                        marco.repaint();
                        
                        JFrame marcoG = new JFrame("Escala de infrarojo");
                        marcoG.getContentPane().add(new JLabel(new ImageIcon(imgNG)));
                        marcoG.setSize(ancho, alto);
                        marcoG.setVisible(true);
                        marcoG.repaint();
                    }
